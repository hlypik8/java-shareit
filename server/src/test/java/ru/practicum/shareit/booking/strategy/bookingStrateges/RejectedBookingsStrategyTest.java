...
class RejectedBookingsStrategyTest {
    private final RejectedBookingsStrategy strategy = new RejectedBookingsStrategy();
    ...
    void getBookings_shouldReturnFromRepository() {
        ...
        when(repository.findAllByStatusAndBooker(BookingStatus.REJECTED, user, sort))
                .thenReturn(List.of(booking));

        List<Booking> result = strategy.getBookings(repository, user, sort, now);

        assertThat(result).containsExactly(booking);
        verify(repository).findAllByStatusAndBooker(BookingStatus.REJECTED, user, sort);
    }

    @Test
    void getBookingsForOwner_shouldReturnFromRepository() {
        Booking booking = new Booking();
        when(repository.findAllByStatusAndItemOwner(BookingStatus.REJECTED, 1, sort))
                .thenReturn(List.of(booking));

        List<Booking> result = strategy.getBookingsForOwner(repository, 1, sort, now);

        assertThat(result).containsExactly(booking);
        verify(repository).findAllByStatusAndItemOwner(BookingStatus.REJECTED, 1, sort);
    }
}
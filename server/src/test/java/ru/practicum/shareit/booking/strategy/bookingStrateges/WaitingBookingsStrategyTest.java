...
class WaitingBookingsStrategyTest {
    private final WaitingBookingsStrategy strategy = new WaitingBookingsStrategy();
    ...
    void getBookings_shouldReturnFromRepository() {
        ...
        when(repository.findAllByStatusAndBooker(BookingStatus.WAITING, user, sort))
                .thenReturn(List.of(booking));

        List<Booking> result = strategy.getBookings(repository, user, sort, now);

        assertThat(result).containsExactly(booking);
        verify(repository).findAllByStatusAndBooker(BookingStatus.WAITING, user, sort);
    }

    @Test
    void getBookingsForOwner_shouldReturnFromRepository() {
        Booking booking = new Booking();
        when(repository.findAllByStatusAndItemOwner(BookingStatus.WAITING, 1, sort))
                .thenReturn(List.of(booking));

        List<Booking> result = strategy.getBookingsForOwner(repository, 1, sort, now);

        assertThat(result).containsExactly(booking);
        verify(repository).findAllByStatusAndItemOwner(BookingStatus.WAITING, 1, sort);
    }
}
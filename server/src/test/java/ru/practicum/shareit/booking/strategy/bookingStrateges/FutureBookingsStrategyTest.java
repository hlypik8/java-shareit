...
class FutureBookingsStrategyTest {
    private final FutureBookingsStrategy strategy = new FutureBookingsStrategy();
    ...
    void getBookings_shouldCallRepositoryAndReturnResult() {
        ...
        when(repository.findAllByBookerAndStartAfter(user, now, sort)).thenReturn(List.of(booking));

        List<Booking> result = strategy.getBookings(repository, user, sort, now);

        assertThat(result).containsExactly(booking);
        verify(repository).findAllByBookerAndStartAfter(user, now, sort);
    }

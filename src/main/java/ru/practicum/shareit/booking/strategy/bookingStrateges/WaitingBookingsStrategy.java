...
@Component
public class WaitingBookingsStrategy implements BookingStrategy {
    @Override
    public List<Booking> getBookings(BookingRepository repository, User user, Sort sort, LocalDateTime now) {
        return repository.findAllByStatusAndBooker(BookingStatus.WAITING, user, sort);
    }

    @Override
    public List<Booking> getBookingsForOwner(BookingRepository repository, Integer userId, Sort sort, LocalDateTime now) {
        return repository.findAllByStatusAndItemOwner(BookingStatus.WAITING, userId, sort);
    }
}
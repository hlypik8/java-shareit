...
@Component
public class CurrentBookingsStrategy implements BookingStrategy {
    @Override
    public List<Booking> getBookings(BookingRepository repository, User user, Sort sort, LocalDateTime now) {
        return repository.findAllByBookerAndStartBeforeAndEndAfter(user, now, now, sort);
    }

    @Override
    public List<Booking> getBookingsForOwner(BookingRepository repository, Integer userId, Sort sort, LocalDateTime now) {
        return repository.findAllByItemOwnerAndStartBeforeAndEndAfter(userId, now, now, sort);
    }
}
